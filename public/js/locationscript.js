...
const locationHandler = async (event) => {
  ...
  async function searchResApi() {
    ...
    async function getlatiandlongi(cityname) {
      ...
      await fetch(currentweatherapi).then((response) => {
        if (response.ok) {
    ...
    async function getLocationDetail(resUrl) {
      ...
      function renderCard(jsonfile) {
        ...
        for (var i = 0; i < jsonfile.data.length; i++) {
          //could load more by using jsonfile.data.length
          if (jsonfile.data[i].photo) {
            // only show restaurants that have got photo
            try {
